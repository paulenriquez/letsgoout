...
func TestHealthAndStaticAssets(t *testing.T) {
	...
	if index.Code != http.StatusOK ||
		...
		!strings.Contains(index.Body.String(), `<link rel="stylesheet" href="/styles.css?v=78">`) ||
	...
	for _, marker := range []string{".manual-link-copy .copy-link-button { display: none; }", "border-style: solid; user-select: none; -webkit-user-select: none;", "-webkit-user-select: text;", ".manual-link-copy .link-box-value { cursor: text; font-size: 16px; }", ".clickable-link-box .link-box-value { pointer-events: none; }", "gap: 0.35rem; color: var(--primary-button);"} {
	...
	if !strings.Contains(styleText, `input[type="text"], textarea { cursor: text; }`) {
		t.Fatal("text fields do not use the standard text cursor")
	}
